...
如需清理任意位置的旧版或直接安装版桌宠，在仓库根目录执行：

```powershell
.\build-uninstaller.ps1
```

将生成的 `publish-uninstaller\一键卸载.exe` 发给 Windows 用户即可。它可以从任意目录运行；使用 `--install-dir "安装路径"` 指定目标，未指定时会定位历史安装记录，并在发现多个候选路径时要求选择。工具会按路径退出后台进程、清理桌面/开始菜单/开机启动入口、删除安装文件，并显示验证结果。

若从待删除的安装目录内双击该 EXE，它会复制自身到临时目录并启动工作进程；原始引导进程以退出码 `2` 表示“已交接、结果待工作进程返回”，最终成功/失败退出码和状态窗口均来自临时工作进程。

通过 `桌宠安装包.exe` 安装的版本支持使用独立 `一键卸载.exe`：工具按固定 AppId 读取所选安装目录及 Inno `UninstallString`，验证卸载命令位于该目录内后启动标准 Inno 卸载器，并验证目录、快捷方式和注册项均已消失。Windows 的“应用和功能”入口仍可启动同一个标准 Inno 卸载器。安装包不再调用旧的 `uninstall.ps1`；Inno Setup 保留其升级时停止历史进程树的逻辑。
