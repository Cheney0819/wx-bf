...
public sealed class ReleaseContractTests
{
    ...
    [Fact]
    public void WindowsInstructionsDescribeOnlyThePreciseCaptureWorkflow()
    {
        var guide = TestSourceTree.ReadWindowsEasy("使用说明.txt");
        var diagnostic = TestSourceTree.ReadWindowsEasy("诊断测试步骤.txt");

        Assert.Contains("1.5-dev", guide, StringComparison.Ordinal);
        ...
        Assert.Contains("1.5-dev", diagnostic, StringComparison.Ordinal);
